...
public class TransactionHeaderServiceImpl implements TransactionHeaderService{
	...
	@Override
	public List<PaymentDetails> saveTransactionHeader(MultipartFile file, int templateId , String fileName) {
	...
	public List<PaymentDetails> getPaymentDetailsByTransactionIdTemplateId(int transactionId, int templateId){
		...
		try {
    		...
    		txn.begin();
    		templateDetails = entityManager.createNativeQuery("SELECT * FROM payment_details WHERE transaction_header_id="+transactionId+" AND template_id="+templateId).getResultList();
    		txn.commit();
		}
		catch ( Throwable e ) {
